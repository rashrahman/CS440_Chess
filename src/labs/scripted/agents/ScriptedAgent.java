    ...
    extends Agent
{
                                            ...
                                            HistoryView history)
	{
        ...
        List<Integer> goldMines = state.getResourceNodeIds(ResourceNode.Type.GOLD_MINE);
        goldResourceNodeId = goldMines.get(0);
    ...
    if (state.isResourceAt(1, 1)) { 
        ...
        } else if (state.getTurnNumber() <= 10) {
            actions.put(getMyUnitId(), Action.createPrimitiveGather(getMyUnitId(), Direction.SOUTH));
        }
    }
    else if (state.getTurnNumber() < 15) {
        // Action attackAction = new TargetedAction(getMyUnitId(), ActionType.COMPOUNDATTACK, getEnemyUnitId());
        // actions.put(getMyUnitId(), attackAction);
        actions.put(getMyUnitId(), Action.createPrimitiveMove(getMyUnitId(), Direction.EAST));
    } else {
